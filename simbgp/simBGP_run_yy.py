#!/usr/bin/env python
#modify by Yangyan about the route defensive policy
import sys;
import string;
import re;
import time;
	...
	def pathSelection(self, prefix):
		...
		if MAX_PATH_NUMBER > 1:
			...
			if len(self.loc_rib[prefix]) == 0 and len(inpaths) > 0:
				...
				trend = -1;
				change = True;
			elif len(self.loc_rib[prefix]) > 0 and len(inpaths) > 0 and self.loc_rib[prefix][0].compareTo4(inpaths[0]) != 0:
...
class CPath:
	...
	def compareTo4(self, path2):
		result = self.compareTo2(path2)
		if result != 0:
			return result;
		if self.nexthop != path2.nexthop:
			return 0

...
class CRouter:
    ...
    origin_rib = None;
    next_idle_time = None; # the time to process the next update, guarantee process in order
    mrai = None;
    mrai_setting = None;
...
#################################################################################
    ...
    def importAction(self, pid, prefix, path):
        ...
        if self.getPeerLink(pid).ibgp_ebgp() == EBGP_SESSION:
            ...
            newpath.igp_cost = 0;
            # insert this router's as number
    ...
    def sendto(self, pid, prefix): # from out_queue
        ...
        if sendsth:
            ...
            #MRAI timer is reset after send a queue of update
            if self.mrai_setting == MRAI_PEER_BASED:
                prefix = None;
